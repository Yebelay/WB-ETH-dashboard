...
load_and_process <- function(data_dir = "data") {
  ...
  # ── Final output list ──────────────────────────────────
  # ── Team A-F summary (D.team6 in dashboard) ─────────────
  mse_by_team <- mse_q %>%
    left_join(mse_lookup %>% select(mse_id, team), by = "mse_id") %>%
    mutate(team = replace_na(team, "TBD")) %>%
    group_by(team) %>%
  ...
  # ── Random Choice Experiment (RCE) ─────────────────────
  # 4 blocks x 6 main questions + 1 repeat check each.
  # repeat_q re-asks one of the 6 main Qs. mismatch = different answer on repeat.
  # flag_uniform = one choice >=80% of the time (possible straight-lining).
  RCE_BLOCKS <- list(
    list(block = 1L,
         qs       = c("choice_set_2","choice_set_6","choice_set_11",
                      "choice_set_21","choice_set_22","choice_set_24"),
         orig     = "choice_set_6",
         repeat_q = "choice_set_6_2"),
    list(block = 2L,
         qs       = c("choice_set_7","choice_set_10","choice_set_13",
                      "choice_set_14","choice_set_19","choice_set_20"),
         orig     = "choice_set_10",
         repeat_q = "choice_set_10_2"),
    list(block = 3L,
         qs       = c("choice_set_8","choice_set_9","choice_set_15",
                      "choice_set_16","choice_set_17","choice_set_18"),
         orig     = "choice_set_9",
         repeat_q = "choice_set_9_2"),
    list(block = 4L,
         qs       = c("choice_set_1","choice_set_3","choice_set_4",
                      "choice_set_5","choice_set_12","choice_set_23"),
         orig     = "choice_set_3",
         repeat_q = "choice_set_3_2")
  )
  ...
  if (length(missing_rce) > 0) {
    ...
    # Compute mismatch BEFORE pivot_longer so blk$orig still exists
    # under its original name (pivot_longer would otherwise drop/rename it).
    # ── Step A: mismatch table at RESPONDENT level (before pivot_longer) ──────
    # BUG FIX: mismatch was previously carried through pivot_longer, which fans
    # each respondent row into 6 rows (one per question). This caused
    # sum(mismatch) to be 6x too large. Fix: compute and aggregate mismatch
    # in a separate respondent-level pipeline, then join onto the freq summary.
    rce_mismatch <- purrr::map_dfr(RCE_BLOCKS, function(blk) {
      mse_q %>%
        ...
        mutate(
          block      = blk$block,
          orig_ans   = toupper(str_trim(as.character(orig_ans))),
          repeat_ans = toupper(str_trim(as.character(repeat_ans))),
          mismatch   = case_when(
    ...
    # ── Step B: A/B/C frequency table (pivot_longer; no mismatch here) ────────
    rce_long <- purrr::map_dfr(RCE_BLOCKS, function(blk) {
      mse_q %>%
        ...
        mutate(response = toupper(str_trim(as.character(response)))) %>%
        select(enumerator, supervisor, block, question, response)
    })

    # ── Step C: aggregate frequencies then join correct mismatch counts ────────
    rce_enum <- rce_long %>%
