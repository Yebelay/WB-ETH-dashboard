...
load_and_process <- function(data_dir = "data") {
  ...
  # ── Team A–F summary (individual teams, used by dashboard team6) ───────
  # All aggregations are driven by the woreda→team lookup in mse_lookup,
  # so no dependency on supervisor names.
  ...
  # Complete (6/6) and incomplete MSE counts per team
  ...
  # Spouse complete (2/2) and partial (1/2) counts per team
  sp_comp_by_team <- per_sp %>%
    group_by(team) %>%
    summarise(
      sp_comp   = sum(sp_complete),
      sp_incomp = sum(sp_followup),
      .groups   = "drop"
    )

  # Follow-up counts per team (MSEs needing action)
  fu_mse_by_team <- per_mse %>%
    filter(followup) %>%
    count(team, name = "fu_mse")

  fu_sp_by_team <- per_sp %>%
    filter(sp_followup) %>%
    count(team, name = "fu_sp")

  # Join all pieces; keep canonical team order A → F
  team6_sum <- tibble(team = c("Team A","Team B","Team C","Team D","Team E","Team F")) %>%
    ...
    replace_na(list(
      i = 0L, m = 0L, si = 0L, sm = 0L,
      comp = 0L, incomp = 0L,
      sp_comp = 0L, sp_incomp = 0L,
      fu_mse = 0L, fu_sp = 0L
    ))
